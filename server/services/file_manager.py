...
# Rutas absolutas basadas en la ubicación de este archivo, para que no
# dependan del directorio desde el que se ejecuta (gunicorn, Docker, etc.).
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # -> server/
DATA_DIR = os.path.join(BASE_DIR, 'data')
RUTA_ARCHIVO_ITEMS = os.path.join(DATA_DIR, 'devices.json')

# Asegurarse de que el directorio y el archivo JSON existan o crearlos.
...
def guardar_historico_sensor(device_id, data):
    file_name = os.path.join(DATA_DIR, f"{device_id}_historico.json")
    nuevo_registro = {
...
def create_historic_file(device_id):
    ...
    file_path = os.path.join(DATA_DIR, f"{device_id}_historico.json")
